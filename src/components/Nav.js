  ...
  const menuItemClick = (setting) => {
    switch (setting) {
      ...
      case "Orders":
        ...
        break;
      case "Logout":
        dispatch(logout()).then(() => navigateTo("/login"));
        break;
    }
    handleCloseUserMenu();
  };
          ...
          <Box sx={{ flexGrow: 0 }}>
            {!auth ? (
              ...
              <Grid container spacing={3} alignItems="center">
                <Grid item>
                  <IconButton onClick={() => navigateTo("/cart")} sx={{ p: 0 }}>
                    <Badge
                      badgeContent={cart.lineItems?.reduce(
            ...
            >
              {settings.map((setting) => (
                <MenuItem key={setting} onClick={() => menuItemClick(setting)}>
